...
import com.ftable21.android.weather.entity.DailyForecast;
import com.ftable21.android.weather.entity.Now;
import com.ftable21.android.weather.entity.WeatherService;
import com.ftable21.android.weather.util.UiUtil;

import java.util.Calendar;

public class SunWinView extends View {
    ...
    public SunWinView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        density = context.getResources().getDisplayMetrics().density;
        mDashPathEffect = new DashPathEffect(new float[] { density * 3, density * 3}, 1);
        paint.setColor(Color.WHITE);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(density);
        paint.setTextAlign(Paint.Align.CENTER);
        if (isInEditMode()) { return; }
        paint.setTypeface(WeatherApplication.getTypeface(context));
    }

    public void setData(WeatherService data) {
        this.now = data.mNow;
        final DailyForecast forecast = data.mDailyForecastArrayList.get(0);
        if (forecast != null) { todayForecast = forecast; }
        if (this.now != null || this.todayForecast != null) { invalidate(); }
    }

    @Override
    protected void onDraw(Canvas canvas) {
        if (this.todayForecast == null || this.now == null) { return; }
        paint.setColor(Color.WHITE);
        ...
        try {
            ...
            canvas.drawText(now.windSpd + "km/h " + now.windDir, fanHeight + textSize, 0, paint);
            paint.setStyle(Paint.Style.STROKE);
            canvas.drawPath(fanPillarPath, paint);
            canvas.rotate(curRotate * 360f);
            float speed = 0f;
            try {
                speed = Float.valueOf(now.windSpd);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            canvas.drawText("大气压强 " + now.AtmosphericPres + "hpa", pressureTextRight, sunArcHeight + paintTextOffset, paint);
            final float textLeft = width / 2f - sunArcRadius;
            paint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText(todayForecast.sunRise, textLeft, textSize * 10.5f + paintTextOffset, paint);
            canvas.drawText(todayForecast.sunDown, width - textLeft, textSize * 10.5f + paintTextOffset, paint);
        } catch (Exception e) {
            e.printStackTrace();
        }

        try {
            String[] sr = todayForecast.sunRise.split(":");
            int srTime = Integer.valueOf(sr[0]) * 60 * 60 + Integer.valueOf(sr[1]) * 60 + 0;
            String[] ss = todayForecast.sunDown.split(":");
